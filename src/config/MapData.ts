...
export function generateTestMap(floorConfig?: { cols: number; rows: number; useAtlas: boolean }): void {
  ...
  if (useAtlas) {
    ...
    // Portal tiles are now generated by PortalManager based on JSON data
    // No hardcoded portal positions here anymore
    
  } else {
    // Floors 1-99 - Green chessboard pattern (all tiles are passable floor)
    // Portal tiles are generated by PortalManager based on JSON data
    MAP_DATA.fill(0);
    MAP_TILE_DATA.fill(0);
